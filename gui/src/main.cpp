...
int real_main(int argc, char *argv[])
{
	...
	QGuiApplication::setOrganizationName("CloudGTA");
	QGuiApplication::setApplicationName("CloudGTAPlayer");
	QGuiApplication::setApplicationVersion(CHIAKI_VERSION);
	QGuiApplication::setApplicationDisplayName("CloudGTA Player");
...
#endif
		QGuiApplication::setDesktopFileName("cloudgta-player");
    ...
    SDL_SetHint(SDL_HINT_APP_NAME, "CloudGTA Player");
	...
	if(!settings.GetCurrentProfile().isEmpty())
		QGuiApplication::setApplicationDisplayName(QString("CloudGTA Player: %1").arg(settings.GetCurrentProfile()));
